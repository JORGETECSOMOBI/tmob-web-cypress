...
class ListaDeVouchersPage {
    ...
    filtroPeriodo() {
        ...
        cy.get('input[placeholder="Data inicial"]').click().type('19/12/1963').type(`{enter}`)
        cy.get('input[placeholder="Data final"]').click().type(data).type(`{enter}`)
        cy.get('.ant-table-cell').invoke('text').should('match', regexData)
        return this
    }

    filtroDataDeExpiracao() {
        cy.get('.ant-row-bottom > :nth-child(3)').click().type('29/02/2024').type(`{enter}`)
